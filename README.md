# Hey, I'm Zhanara 👋
**Data Analyst · Data Scientist · Business Analyst**

Data-driven professional with strong experience across analytics, CX quality, auditing, and financial analysis. I specialize in transforming complex business problems into actionable insights through data modeling, automation, and visualization. Currently working in Prague, focusing on scalable analytics solutions that improve decision-making and operational efficiency.

📍 Prague, Czech Republic  

---

## 🚀 What I Do
- **Data Analytics & Insights** → KPI frameworks, trend analysis, root-cause analysis, decision support
- **Data Visualization** → Executive-level BI dashboards and stakeholder reporting
- **Process Automation** → Power Automate, SQL logic, Salesforce data clean-up, workflow optimization
- **Data Quality & Governance** → Data stewardship, validation rules, system reliability

---

## 🏆 Key Achievements
- Led **CX quality programs and analytics stream** across a global Order‑to‑Cash function at Honeywell
- Built **11+ Power BI dashboards**, including end‑to‑end executive dashboards, reducing time‑to‑insight
- Automated OTC processes resulting in **~95% manual effort reduction** and **90–95% accuracy**
- Improved Salesforce data quality, **cutting up to 2,000 bad records per day**
- Increased collections productivity by **30%+** through SQL-driven analysis and system redesign

---

## 📝 Selected Projects & Case Studies
- **Finance & Data Analytics Thesis**  
  *Impact of Macroprudential Policy Announcements on Financial Markets*  
  R-based econometric analysis on European markets, analyzing effects on bond yields, stock prices, and volatility.

- **British Airways – Data Science Job Simulation (Forage)**  
  Sentiment analysis and topic modeling on Skytrax reviews; built predictive models for booking behavior.  
  *Tools: Python*

- **Accenture – Data Analytics & Visualization Simulation**  
  Big data audit and content trend analysis using Power BI & DAX.

- **KPMG – Data Analytics Consulting Virtual Internship**  
  Customer segmentation, geo-analysis, and dual marketing strategy design.  
  *Tools: Power BI, DAX*

- **Google Data Analytics Capstone**  
  Bellabeat smart device usage analysis with actionable business recommendations.  
  *Tools: R*

🔗 **More projects:**  
- https://github.com/ZhanaraZeinesheva/R-and-Python-Projects
- https://github.com/ZhanaraZeinesheva/automation-with-python

---

## 🛠 Tech Stack
**Analytics & BI**  
Power BI · DAX · Tableau · SQL · Excel

**Programming & Data Science**  
Python · R · Machine Learning · Econometrics

**Enterprise Systems**  
Salesforce · SAP · Power Automate

**Concepts**  
Data Modeling · Data Quality · UAT · Lean & Six Sigma · KPI Design

---

## 📜 Certifications
- Google Data Analytics Professional Certificate
- Microsoft Power BI Data Analyst (PL-300 / DA-100)
- Lean & Six Sigma Green Belt
- Google IT Automation with Python
- McKinsey Forward Learner

---

## 🌍 Languages
- Kazakh — Native  
- Russian — Native  
- English — Fluent  
- Italian — Intermediate  
- Czech — Beginner  

---

## 📫 Let’s Connect
- 📧 Email: zhanara.zeinesheva@gmail.com  
- 💼 LinkedIn: https://www.linkedin.com/in/zhanara-zeinesheva-618236a3/  

💼 **Open to Data Analyst, Data Scientist, and Business Analyst roles** — Prague, Czechia / on-site, hybrid, remote; EU, UK / remote
